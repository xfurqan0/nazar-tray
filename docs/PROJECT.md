...
| T-WP-L12 ✅ | **Feature parity on Linux, and the two doors to the settings**: a gear in the panel's header on every platform, `Usage history` in the tray menu, the panel window held at the size it asks for on GTK, a close that hides rather than ends the run, and `tray.showLabel` / `window.x11Positioning` on the settings page where only a Linux build draws them | ~~Every user-visible Windows feature is reachable on Linux or written down as meaningless there; the menu reads Open panel · Settings… · Usage history · Refresh now · Quit in all six languages; the panel is as tall as its content rather than as tall as its document; closing the window leaves the tray and the engine running; a Windows `config.json` still does not change a byte~~ — **landed 2026-09-17** |
...
- ~~The panel's position on Wayland.~~ → **closed 2026-09-17 by T-WP-L8: it is not placed, and the numbers move to the surface that is.** The measurement first, on Fedora 44 / GNOME 50: `set_position` returns `Ok(())` twice and `outer_position()` never leaves `(0, 0)`; `cursor_position()` returns `Ok((0, 0))` rather than an error, so the arithmetic about which side of the cursor to open on was running on a lie. Under `GDK_BACKEND=x11` the identical run landed the window on exactly `(700, 400)` and read a real pointer — so XWayland is the lever, and it is `config.window.x11Positioning`, off by default, because it puts the whole application including the webview on an X11 translation layer to move one window.
  ...
  - ~~**`x11Positioning` in the settings form, and whether the panel hides on blur.**~~ → **the first is closed 2026-09-17 by T-WP-L12: it is a row on the settings page, on Linux builds only.** A key that exists only in `config.json` is a feature shipped one screen short of the user it was written for, and the two Linux-only keys — the label beside the bead and the panel through XWayland — are exactly the two settings a Linux user would look for and not find. They are drawn behind `desktop::DESKTOP_SWITCHES`, the mirror of the constant that keeps the Windows overflow hint off a Linux panel, and the cost of the XWayland one is in the sentence under it rather than only in the README. **The blur question stays open** and is unchanged: the panel still hides when it loses focus on every platform.
- ~~**What stays open:** a `StatusNotifierWatcher` that appears *later*.~~ → **closed 2026-09-17 by T-WP-L10, which reverses T-WP-L2's decision on a measurement rather than on second thoughts.** The old reasoning was that a watcher arriving mid-run meant the AppIndicator extension being installed, and that needs the GNOME session restarted on Wayland anyway — a subscription held for the life of every Linux process to save a step the user had already taken. What the premise missed is that **GNOME disables extensions while the session is locked and the watcher's bus name goes with them**: observed at 00:25 on a machine whose watcher had been on the bus at 00:03 with `gnome-shell` on the same pid, and confirmed again at 01:05, when `gnome-extensions disable` and `enable` moved the name not at all because a locked session keeps every extension `INACTIVE`. A tray that starts in that window is not a user who forgot to install something; it is autostart, every morning.
    - **What the subscription does, and what it deliberately does not.** `desktop::watch` holds one `NameOwnerChanged` match rule on that one name, so the bus wakes this process for nothing else. A host arriving where there is no icon builds one — the whole package. A host arriving where there *is* one does nothing, because libappindicator keeps its own watch on the same name and re-registers the item itself; a second `install` would be a second `TrayIcon` under one id and a `QuotaRows` no refresh writes to. A host leaving records that nothing is drawing the icon, says so on standard error, and touches neither the item nor the engine. **And no transition is ever notified**: the one-off "no tray host" notice belongs to the start-up question, and a toast every time somebody unlocks a working tray is the version of this feature that is worth less than nothing.
    - **Measured on this machine, with the screen locked.** A stand-in watcher owned the name on demand, which is the only way to move it on a locked session. `nazar-tray` started in engine mode, the name appeared, the icon went up — `IconName …/tray-icon-nazar-tray-1.png`, `Status Active`, and a dbusmenu carrying the T-WP-L8/L9 rows in Turkish, so what arrives late is the whole face rather than a stripped one. The name went away: one line of log, the process alive, `limits.json` still written. The name came back: **no** second line from us, and a second `RegisterStatusNotifierItem` from libappindicator, which is that claim measured rather than assumed. `dbus-monitor` counted **zero** `Notify` calls across the run.
...
- 2026-09-17 — **T-WP-L7, L8 and L9 landed: three things a Linux first run got wrong.** Reported
  ...
  **One thing found and left open:** GNOME disables extensions while the session is locked, and
  ...
  than it was.
- 2026-09-17 01:05 — **T-WP-L10 landed: the tray survives a lock screen, and T-WP-L2's one
  rejected idea is now the right one.** One `NameOwnerChanged` subscription, narrowed by a match
  rule to `org.kde.StatusNotifierWatcher`, on a thread that spends its life parked in `poll`; a
  pure `desktop::Presence` deciding what each change is worth doing; and the icon built on the
  main thread when a host appears where there was none. **An icon is built once per run** —
  libappindicator holds its own watch on the same name and re-registers the item itself, so a
  second `install` would be a second `TrayIcon` under one id and a `QuotaRows` that no refresh
  writes to. A host *leaving* records the state, says one line on standard error, and leaves the
  item and the engine alone. **No transition is ever notified**: the start-up notice keeps its
  one claim in `alerts.json`, and a toast every time a working tray is unlocked would be worse
  than the bug. Measured on the locked Fedora 44 session that found the bug, with a stand-in
  watcher owning the name on demand: engine mode → name appears → `Status Active` and the full
  Turkish dbusmenu, rows and all → name goes → one line, process alive, `limits.json` still
  written → name returns → nothing from us and a second `RegisterStatusNotifierItem` from
...
- 2026-09-17 09:40 — **T-WP-L12 landed: what the settings page is for, and three things Linux
  had been doing quietly wrong.** The maintainer's report was one sentence — *let us get into
  the settings from a right click, or put a gear on the screen that opens* — and the audit
  behind it was the rest of the package: every user-visible Windows feature against what a
  Fedora 44 / GNOME 50 Wayland session actually does with it, measured on this machine rather
  than reasoned about.
  **What was already right, and is now written down.** Settings *were* reachable from the
  Linux menu: a `dbusmenu` `Event` on the `Ayarlar` item produced `open_settings` → panel shown
  → the settings view measured at 1544 CSS pixels, so the path worked and was merely hard to
  find. Both mouse buttons open that menu, because libappindicator owns the click — the item
  publishes no `Activate` method and no `ItemIsMenu` property, read back over D-Bus. Threshold
  notifications reach GNOME (`--demo-cross`: one toast at 60 %, one at 85 %, **nothing** for
  the repeat, one more after the reset), quiet hours suppress exactly those (the same run with
  a window around the current time: zero threshold toasts, only the one-time GNOME notice), a
  language override repaints the whole menu (`locale: "en"` on a Turkish desktop), switching a
  provider off stops it being read, and the usage view draws a real month of Claude Code
  tokens from `~/.claude/projects`.
  **Three that were wrong.** (1) **The panel window was as tall as its document.** `tao 0.35.3`
  sends every `set_size` to `gtk_window_resize`, which does nothing on a window whose
  `resizable` flag is off — and `tauri.conf.json` says `resizable: false` — so the window took
  the natural height of the web view instead: **1105 physical pixels with 480 of content**,
  measured with `xprop` and `import` on the real window, and it never shrank back. `MIN_HEIGHT`,
  `MAX_HEIGHT` and the whole measure-and-ask arrangement had been Windows-only behaviour
  without anybody deciding that. `panel::pin` makes the GTK window resizable and then pins the
  minimum to the maximum, which is the one instruction GTK honours; the same window is now
  360 × 480. (2) **Closing the panel ended the run.** With no `CloseRequested` handler the only
  window closing takes the process, the tray icon and `limits.json` with it — proven with a
  probe that asked the window to close and then waited for a process that was no longer there.
  It is a bug both platforms had (Alt+F4 reaches a frameless window on Windows too) that only
  a Linux desktop's Super+Q made easy to hit; the window is hidden now, and `Quit` stays the
  way out that releases the lock. (3) **The usage view was not in the tray menu at all**, which
  on Linux — where the panel is one menu item further away than on Windows — put a whole view
  two clicks and a guess away.
  **And the two doors.** The gear is in the panel's header on **both** platforms, because a
  second door to a screen is not a Linux feature; the footer's *Settings* word is gone, so
  there is exactly one control that opens the page and the test says so. The menu is five
  items: `Open panel · Settings… · Usage history · Refresh now`, separator, `Quit` — one
  ordered list in the source rather than an order spelled out twice. Six catalogues, 170 → 176
  keys. `tray.showLabel` and `window.x11Positioning` are rows on the settings page behind
